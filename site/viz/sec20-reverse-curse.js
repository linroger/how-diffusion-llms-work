...
(function () {
  ...
  function render() {
    ...
    prompts.forEach((pr, i) => {
      ...
      btn.style.background = i === promptIdx ? 'var(--accent)' : 'transparent';
      btn.style.color = i === promptIdx ? 'var(--btn-text)' : 'var(--text-soft)';
      btn.style.border = '1px solid ' + (i === promptIdx ? 'var(--accent)' : 'var(--border-strong)');
    ...
    conclusion.style.background = 'var(--bg-frame-2)';
    conclusion.style.borderRadius = '6px';
    conclusion.style.borderLeft = '3px solid var(--accent)';
    conclusion.style.fontFamily = 'Source Serif 4, serif';
    conclusion.style.fontSize = '0.95rem';
    conclusion.style.color = 'var(--text)';
  ...
  function renderCol(title, prompt, answer, arConf, diffConf, lang) {
    ...
    col.style.background = 'var(--bg-frame-2)';
    col.style.borderRadius = '6px';
    col.style.border = '1px solid var(--border-strong)';
    col.innerHTML = `
      <div style="font-family:Inter,sans-serif;font-size:0.74rem;font-weight:600;letter-spacing:0.08em;text-transform:uppercase;color:var(--text-soft);margin-bottom:12px">${title}</div>
      <div style="font-family:JetBrains Mono,monospace;font-size:0.94rem;color:var(--text);margin-bottom:18px;line-height:1.4">${prompt}</div>
      <div style="margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;font-family:Inter,sans-serif;font-size:0.78rem;color:var(--accent-2);margin-bottom:4px">
          <span>AR (GPT-style)</span>
          <span style="font-family:JetBrains Mono,monospace">${(arConf * 100).toFixed(1)}%</span>
        </div>
        <div style="height:8px;background:var(--bg-frame-2);border-radius:4px;overflow:hidden">
          <div style="height:100%;width:${arConf * 100}%;background:var(--accent-2)"></div>
        </div>
        <div style="font-family:JetBrains Mono,monospace;font-size:0.8rem;color:var(--text-soft);margin-top:4px">→ "${answer}"</div>
      </div>
      <div>
        <div style="display:flex;justify-content:space-between;font-family:Inter,sans-serif;font-size:0.78rem;color:var(--accent);margin-bottom:4px">
          <span>Diffusion (LLaDA-style)</span>
          <span style="font-family:JetBrains Mono,monospace">${(diffConf * 100).toFixed(1)}%</span>
        </div>
        <div style="height:8px;background:var(--bg-frame-2);border-radius:4px;overflow:hidden">
          <div style="height:100%;width:${diffConf * 100}%;background:var(--accent)"></div>
        </div>
        <div style="font-family:JetBrains Mono,monospace;font-size:0.8rem;color:var(--text-soft);margin-top:4px">→ "${answer}"</div>
      </div>
    `;
