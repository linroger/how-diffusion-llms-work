...
(function () {
  ...
  function draw() {
    ...
    xGrid.selectAll('line').style('stroke', 'var(--bg-frame-2)').style('stroke-dasharray', '2,3');
    xGrid.select('.domain').remove();
    const yGrid = g.append('g').call(d3.axisLeft(y).ticks(5).tickSize(-iw).tickFormat(''));
    yGrid.selectAll('line').style('stroke', 'var(--bg-frame-2)').style('stroke-dasharray', '2,3');
    yGrid.select('.domain').remove();

    // Axes (labels)
    g.append('g').attr('transform', `translate(0,${ih})`)
      .call(d3.axisBottom(x).ticks(6, '~s'))
      .selectAll('text').style('fill', 'var(--text-soft)').style('font-size', '11px').style('font-family', 'JetBrains Mono, monospace');
    g.append('g')
    ...
    const tt = tooltip.enter().append('div').attr('class', 'tradeoff-tooltip')
      .style('position', 'fixed').style('pointer-events', 'none').style('background', 'var(--bg-frame-2)').style('border', '1px solid var(--border-strong)')
      .style('border-radius', '4px').style('padding', '8px 12px').style('font-family', 'JetBrains Mono, monospace').style('font-size', '12px')
      .style('color', 'var(--text)').style('display', 'none').style('z-index', '1000').merge(tooltip);

    POINTS.forEach((p) => {
      g.append('circle')
        .attr('cx', x(p.tps)).attr('cy', y(p.score)).attr('r', p.size)
        .attr('fill', p.group === 'ar' ? 'var(--accent-2)' : 'var(--accent)')
        .attr('opacity', 0.85)
        .attr('stroke', p.group === 'ar' ? 'var(--accent-2)' : 'var(--accent)').attr('stroke-width', 2)
        .style('cursor', 'pointer')
        .on('mouseenter', function (event) {
          d3.select(this).attr('r', p.size + 3).attr('opacity', 1);
          tt.style('display', 'block').html(`<b>${p.name}</b><br>TPS: ${p.tps}<br>Score: ${p.score}<br><span style="color:var(--text-soft)">${p.group === 'ar' ? 'Autoregressive' : 'Diffusion'}</span>`);
        })
    ...
    g.append('text').attr('x', iw / 2).attr('y', ih + 40).attr('text-anchor', 'middle')
      .style('fill', 'var(--text-soft)').style('font-family', 'JetBrains Mono, monospace').style('font-size', '12px').text('throughput (tokens/sec) — log scale');
    g.append('text').attr('transform', `translate(-44, ${ih / 2}) rotate(-90)`).attr('text-anchor', 'middle')
      .style('fill', 'var(--text-soft)').style('font-family', 'JetBrains Mono, monospace').style('font-size', '12px').text('benchmark score (47-benchmark avg)');

    // Title
    g.append('text').attr('x', 0).attr('y', -10).style('fill', 'var(--text)').style('font-family', 'Source Serif 4, serif').style('font-size', '14px').style('font-weight', '600')
    ...
    g.append('path').datum(frontier).attr('fill', 'none').style('stroke', 'var(--accent)').attr('stroke-width', 1.5).attr('stroke-dasharray', '4,4').attr('opacity', 0.4).attr('d', line);

    // Legend
    const legend = g.append('g').attr('transform', `translate(${iw - 200}, 8)`);
    legend.append('circle').attr('cx', 6).attr('cy', 0).attr('r', 6).style('fill', 'var(--accent-2)');
    legend.append('text').attr('x', 18).attr('y', 4).style('fill', 'var(--text-soft)').style('font-family', 'Inter, sans-serif').style('font-size', '12px').text('Autoregressive');
    legend.append('circle').attr('cx', 6).attr('cy', 18).attr('r', 6).style('fill', 'var(--accent)');
    legend.append('text').attr('x', 18).attr('y', 22).style('fill', 'var(--text-soft)').style('font-family', 'Inter, sans-serif').style('font-size', '12px').text('Diffusion');
  }
